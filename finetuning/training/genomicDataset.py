...
# os.environ[
#     "SM_CHANNEL_RESOURCES"
# ] = "/Users/wejarrard/projects/atacToChip/tf-binding-prediction/finetuning/preprocessing/output"
# os.environ["SM_OUTPUT_DATA_DIR"] = "./output"


def get_tokenizer(tokenizer_path):
    tokenizer = PreTrainedTokenizerFast(tokenizer_file=tokenizer_path, max_len=512)
    return tokenizer
...
class GenomicsDataset(Dataset):
    def __init__(
        ...
        dir_label_dict,
        min_val=0,
        max_val=366.0038259577389,
        dataset="train",
        tokenizer=get_tokenizer(
            os.path.join(os.environ["SM_CHANNEL_RESOURCES"], "tokenizer.json")
        ),
    ):
        ...
        for dir_path, label in dir_label_dict.items():
            files = [
                ...
                if not pd.read_feather(os.path.join(dir_path, file)).empty
            ]
    ...
    def min_max_norm(self, x, scale_max=100):
        min_val = torch.min(x)
        max_val = torch.max(x)
        
        # Check if all samples are the same value (e.g., all zeros)
        if min_val == max_val:
...
if __name__ == "__main__":
    ...
    data_dir = "/Users/wejarrard/projects/atacToChip/tf-binding-prediction/finetuning/testing/synthetic_data/output/train"
    dir_label_dict = {
    ...
    for i, data in enumerate(dataloader):
        ...
        for j, read in enumerate(reads):
            ...
            color = colors[
                labels[j].item()
            ]  # Extract label as Python number and determine color
            if labels[j].item() == 0:
