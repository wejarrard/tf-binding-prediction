...
def process_data(atacseq_bed_path, blacklist_bed_path, pileup_dir, output_path, total_reads):
    ...
    df_atacseq_only = pd.read_table(atacseq_bed.fn, header=None, names=['chrom', 'start', 'end', 'name', 'score', 'strand', 'signalValue', 'pValue', 'qValue'], index_col=False)
    ...
    for chrom in df_atacseq_only['chrom'].unique():
        print(f"Parsing chromosome {chrom}")
        chrom_df = df_atacseq_only[df_atacseq_only['chrom'] == chrom]
        ...
        for split_df, split in [(train_df, "train"), (test_df, "test")]:
            ...
            for i, row in tqdm(split_df.iterrows(), total=split_df.shape[0]):
                ...
                try:
                    feather_df.reset_index(drop=True).to_feather(f"{output_path}/{split}/{sample_name}_{chrom}_{i}.feather")
                except Exception as e:
...
def main():
    ...
    for path in [args.atacseq_bed_path, args.atacseq_bam_path, args.output_path, args.genome_path, args.blacklist_bed_path]:
